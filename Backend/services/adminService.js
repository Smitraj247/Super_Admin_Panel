...
export const addRole = (name, permissions = []) => Role.create({ name, permissions });
...
export const addUser = async (body, requestingUser) => {
  ...
  if (requestingUser.role.name === "ADMIN" && requestingUser.department && !isHRAdmin(requestingUser)) {
    if (department && department !== requestingUser.department._id.toString()) {
      throw Object.assign(new Error("Admins can only create users in their own department"), { status: 403 });
    }
  ...
  if (pwd.length < 6) throw Object.assign(new Error("Password must be at least 6 characters"), { status: 400 });
...
export const editUser = async (id, body, requestingUser, departmentFilter) => {
  ...
  if (requestingUser.role.name === "ADMIN" && requestingUser.department && !isHRAdmin(requestingUser)) {
    if (user.department.toString() !== requestingUser.department._id.toString()) {
      throw Object.assign(new Error("You can only manage users in your own department"), { status: 403 });
    }
    if (department && department !== requestingUser.department._id.toString()) {
      throw Object.assign(new Error("Cannot change user to a different department"), { status: 403 });
    }
  }
...
export const removeUser = async (id, requestingUser) => {
  ...
  if (requestingUser.role.name === "ADMIN" && requestingUser.department && !isHRAdmin(requestingUser)) {
    if (user.department.toString() !== requestingUser.department._id.toString()) {
      throw Object.assign(new Error("You can only delete users in your own department"), { status: 403 });
    }
  }
...
export const listAdmins = async (requestingUser) => {
  ...
  const filter = { role: adminRole._id };
  if (requestingUser.role.name === "ADMIN" && requestingUser.department && !isHRAdmin(requestingUser)) {
...
export const addAdmin = async (body, requestingUser) => {
  ...
  if (requestingUser.role.name === "ADMIN" && requestingUser.department && !isHRAdmin(requestingUser)) {
    if (department !== requestingUser.department._id.toString()) {
      throw Object.assign(new Error("You can only create admins in your own department"), { status: 403 });
    }
  }

  const { createUser } = await import("./userService.js");
  const user = await createUser(name, email, password || "123456", "ADMIN", department, requestingUser._id);
...
export const editAdmin = async (id, body, requestingUser) => {
  ...
  if (requestingUser.role.name === "ADMIN" && requestingUser.department && !isHRAdmin(requestingUser)) {
    if (admin.department.toString() !== requestingUser.department._id.toString()) {
      throw Object.assign(new Error("You can only update admins in your own department"), { status: 403 });
    }
  }
...
export const removeAdmin = async (id, requestingUser) => {
  ...
  if (requestingUser.role.name === "ADMIN" && requestingUser.department && !isHRAdmin(requestingUser)) {
    if (admin.department.toString() !== requestingUser.department._id.toString()) {
      throw Object.assign(new Error("You can only delete admins in your own department"), { status: 403 });
    }
  }
