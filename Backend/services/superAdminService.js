...
export const listAdmins = async () => {
  ...
  return User.find({ role: adminRole._id, isActive: true })
    .populate("role", "name")
...
export const addDepartment = async (name) => {
  if (await Department.findOne({ name }))
    throw Object.assign(new Error("Department already exists"), {
      status: 400,
    });
...
export const getDashboardStats = async () => {
  ...
  const departmentUsage = departments.map((d) => {
    const match = deptCounts.find(
      (c) => c._id?.toString() === d._id.toString(),
    );
