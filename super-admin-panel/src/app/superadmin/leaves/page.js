...
export default function SuperAdminLeaves() {
  ...
  const fetchUserLeaves = async (userId) => {
    setLoading(true);
    try {
      const res = await getUserLeaveHistoryApi(userId, selectedYear, selectedMonth);
      const data = res.data?.data || res.data || {};
  ...
  useEffect(() => {
    ...
    if (departmentFilter) {
      filtered = filtered.filter((user) => user.department?._id === departmentFilter);
    }
                ...
                {selectedUser.leaveBalance && (
                  <div className="grid grid-cols-4 gap-4 mt-4">
                    <div className="bg-blue-50 p-4 rounded-lg">
                      <p className="text-sm text-blue-600 font-semibold">Privilege Leave</p>
                      <p className="text-2xl font-bold text-blue-700">{selectedUser.leaveBalance.PL || 0}</p>
                    </div>
                    <div className="bg-green-50 p-4 rounded-lg">
                      <p className="text-sm text-green-600 font-semibold">Casual Leave</p>
                      <p className="text-2xl font-bold text-green-700">{selectedUser.leaveBalance.CL || 0}</p>
                    </div>
                    <div className="bg-yellow-50 p-4 rounded-lg">
                      <p className="text-sm text-yellow-600 font-semibold">Sick Leave</p>
                      <p className="text-2xl font-bold text-yellow-700">{selectedUser.leaveBalance.SL || 0}</p>
                    </div>
                    <div className="bg-purple-50 p-4 rounded-lg">
                      <p className="text-sm text-purple-600 font-semibold">Duty Leave</p>
                      <p className="text-2xl font-bold text-purple-700">{selectedUser.leaveBalance.DL || 0}</p>
                    </div>
                  </div>
                )}
              </div>
